            ...
            props.ForEach(p =>
                              {
                                  ...
                                  if (val != null && val.GetType().Name.Contains("AnonymousType"))
                                      dic.Add(p.Name, ToDictionary(val));
                                  else
                                      dic.Add(p.Name,
                                              p.PropertyType.GetInterface(typeof (IEnumerable).Name) != null &&
                                              p.PropertyType != typeof (string)
                                                  ? ToDictionary(p.Name, (IEnumerable) p.GetValue(message, null))
                                                  : (p.PropertyType.IsClass && p.PropertyType != typeof (string) &&
                                                     p.PropertyType != typeof (Exception) &&
                                                     p.PropertyType != typeof (object)
                                                         ? ToDictionary(p.GetValue(message, null))
                                                         : p.GetValue(message, null)));
                              }
                );
